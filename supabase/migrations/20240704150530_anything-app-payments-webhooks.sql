...
DECLARE
  user_data json;
BEGIN
--   -- Check if the function is being called by the service role
--   IF NOT (SELECT rolname FROM pg_roles WHERE rolsuper AND rolname = current_user) THEN
--     RAISE EXCEPTION 'Access denied. This function can only be executed by the service role.';
--   END IF;
