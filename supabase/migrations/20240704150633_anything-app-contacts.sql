...
CREATE TABLE IF NOT EXISTS anything.contacts
(
    contact_id uuid unique NOT NULL DEFAULT uuid_generate_v4() primary key,
    ...
    -- CRM specific fields
    status TEXT, -- e.g. lead, customer, prospect
...
CREATE TRIGGER set_contacts_timestamp
    BEFORE INSERT OR UPDATE ON anything.contacts
    FOR EACH ROW
EXECUTE PROCEDURE basejump.trigger_set_timestamps();

-- protect the updated_by and created_by columns by setting them to be read-only and managed by a trigger
CREATE TRIGGER set_contacts_user_tracking
    BEFORE INSERT OR UPDATE ON anything.contacts
    FOR EACH ROW
...
ALTER TABLE anything.contacts ENABLE ROW LEVEL SECURITY;
...
----------------
-- create policy "All logged in users can select" on anything.contacts
--     for select
--     to authenticated
--     using (true);
...
----------------
-- create policy "All authenticated and anonymous users can select" on anything.contacts
--     for select
...
--------------
create policy "Account members can select" on anything.contacts
    for select
    to authenticated
    using (
...
----------------
create policy "Account members can insert" on anything.contacts
    for insert
    to authenticated
    with check (
    (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
---------------
create policy "Account members can update" on anything.contacts
    for update
    to authenticated
    using (
    (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account members can delete" on anything.contacts
--     for delete
--     to authenticated
--     using (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account owners can delete" on anything.contacts
--     for delete
--     to authenticated
--     using (
