...
pub fn register() -> FnResult<AnythingPlugin> {
    let plugin: AnythingPlugin = AnythingPlugin::builder()
        .trigger(false)
        .label("Loop Through Something".to_string())
        ...
        .description("Loops through a list of items to get more work done simply.".to_string())
        .variables(vec![])
...
pub fn execute(config: Value) -> FnResult<Value> {
    //TODO: Create new events from items in the input array

    //Ok so i think the way this can work is ->
    //You query the input of the next action in the flow
    
    //you could provide an api for plugins to set a variable?
    //set variable
