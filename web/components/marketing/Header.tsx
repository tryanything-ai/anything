...
export function Header({ stargazers_count }: { stargazers_count: number }) {
  ...
  return (
    <header className="border-b border-primary bg-opacity-20 py-3 lg:py-0">
      <nav
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-10 bg-opacity-70">
          <div className="fixed right-0 z-20 w-full sm:max-w-sm sm:ring-1 sm:ring-primary p-6">
            ...
            <div className="mt-6">
              {navigation.map((item) => (
                <Link key={item.name} href={item.href}>
                  <span className="btn btn-ghost block w-full my-2">
                    {item.name}
                  </span>
                </Link>
              ))}
              <SignInButton className="btn w-full" />
              <SignUpButton className="btn w-full mt-2" />
            </div>
          </div>
        </div>
      )}
