...
CREATE TRIGGER set_flow_templates_timestamp
    BEFORE INSERT OR UPDATE ON public.flow_templates
    FOR EACH ROW
...
----------------
create policy "All logged in users can select" on public.flow_templates
    for select
    to authenticated
    using (true);
...
----------------
create policy "All authenticated and anonymous users can select" on public.flow_templates
    for select
...
--------------
-- create policy "Account members can select" on public.flow_templates
--     for select
--     to authenticated
--     using (
...
----------------
-- create policy "Account members can insert" on public.flow_templates
--     for insert
--     to authenticated
--     with check (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
---------------
-- create policy "Account members can update" on public.flow_templates
--     for update
--     to authenticated
--     using (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account members can delete" on public.flow_templates
--     for delete
--     to authenticated
--     using (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account owners can delete" on public.flow_templates
--     for delete
--     to authenticated
--     using (
