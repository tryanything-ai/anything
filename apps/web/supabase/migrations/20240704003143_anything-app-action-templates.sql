...
CREATE TABLE IF NOT EXISTS anything.action_templates
(
...
CREATE TRIGGER set_action_template_timestamp
    BEFORE INSERT OR UPDATE ON anything.action_templates
    FOR EACH ROW
EXECUTE PROCEDURE basejump.trigger_set_timestamps();

-- protect the updated_by and created_by columns by setting them to be read-only and managed by a trigger
CREATE TRIGGER set_action_template_user_tracking
    BEFORE INSERT OR UPDATE ON anything.action_templates
    FOR EACH ROW
...
ALTER TABLE anything.action_templates ENABLE ROW LEVEL SECURITY;
...
----------------
-- create policy "All logged in users can select" on anything.action_templates
--     for select
--     to authenticated
--     using (true);
...
----------------
-- create policy "All authenticated and anonymous users can select" on anything.action_templates
--     for select
...
--------------
create policy "Account members can select" on anything.action_templates
    for select
    to authenticated
    using (
...
----------------
create policy "Account members can insert" on anything.action_templates
    for insert
    to authenticated
    with check (
    (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
---------------
create policy "Account members can update" on anything.action_templates
    for update
    to authenticated
    using (
    (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
create policy "Account members can delete" on anything.action_templates
    for delete
    to authenticated
    using (
    (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account owners can delete" on anything.action_templates
--     for delete
--     to authenticated
--     using (
