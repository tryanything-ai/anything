...
pub async fn process_response_task(
    ...
    let status_code = bundled_context.get("status_code")
        .and_then(|v| v.as_str())
        .unwrap_or("200");

    let content_type = bundled_context.get("content_type")
        .and_then(|v| v.as_str())
        .unwrap_or("application/json");

    let headers = bundled_context.get("headers")
        .and_then(|v| v.as_str())
        .unwrap_or("");

    let body = bundled_context.get("body")
        .and_then(|v| v.as_str())
        .unwrap_or("");

    // Build response object
    let mut response = serde_json::Map::new();
    response.insert("status_code".to_string(), Value::String(status_code.to_string()));
    response.insert("content_type".to_string(), Value::String(content_type.to_string()));
    
    // Parse and add headers if present
    if !headers.is_empty() {
        match deep_parse_json(headers) {
            Ok(parsed_headers) => {
                response.insert("headers".to_string(), parsed_headers);
            },
            Err(e) => {
                println!("[PROCESS RESPONSE] Failed to parse headers: {}", e);
                response.insert("headers".to_string(), Value::String(headers.to_string()));
            }
        }
    }
