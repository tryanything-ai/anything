...
pub async fn process_formatter_task(
    bundled_context: &Value,
) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
    println!("[FORMATTER] Starting process_formatter_task");
    println!("[FORMATTER] Initial bundled context: {:?}", bundled_context);

    let input_type = bundled_context
        .get("input_type")
        .and_then(|v| v.as_str())
        .unwrap_or("text");
    ...
    // Process based on input type and operation
    let result = match input_type {
        "date" => {
            ...
            let parsed_date = if let Ok(dt) = DateTime::parse_from_rfc3339(input) {
                ...
                return Ok(
                    json!({"result": "Invalid date format. Please use RFC3339 or YYYY-MM-DD HH:MM:SS"}),
                );
            };
    ...
    response.insert("result".to_string(), Value::String(result));

    println!("[FORMATTER] Generated response: {:?}", response);
